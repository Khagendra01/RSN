...
class CRRL{
			...
			if(!this.writeToLog){
				try{
					...
					this.writeToLog=function(x){
						this.logfileStream.write(`${Date.now()}\t${JSON.toJSON(x)}\n`);
					}
				}catch(e){
					if(e instanceof ReferenceError){
						this.writeToLog=function(x){
							console.log(`${Date.now()}\t${JSON.toJSON(x)}`);
						}
					}else throw e;
				}
			}
	...
	expose(inputs,repeats=10,progress){
		var curLearning=this.learning;
		this.learning=false;
		for(var i=0;i<inputs.length;i++){
			for(var j=0;j<repeats;j++){
				this.step(inputs[i],this.allowedActions[0]);
				this.learnChunking();
			}
			if(progress && !((i+1)%progress))
				console.log(i,'chunks',Object.keys(this.chunks).length);
		}
		this.learning=curLearning;
	}
	predict(inputs){
